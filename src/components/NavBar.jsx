...
const generateRandomNotif = () => ({ id: Math.random(), text: "New notification" });

const Notification = ({ text, id, removeNotif }) => {
  useEffect(() => {
    const timeoutRef = setTimeout(() => removeNotif(id), 10000);
    return () => clearTimeout(timeoutRef);
  }, [id, removeNotif]);

  return (
    <div className="p-2 flex items-center justify-center rounded gap-2 text-xs font-medium shadow-lg bg-[#202020] hover:bg-[#171717] text-white px-5 py-2.5 pointer-events-auto">
      <span>{text}</span>
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-1 w-72 fixed top-2 right-2 z-50 pointer-events-none">
        {notifications.map((n) => <Notification removeNotif={removeNotif} {...n} key={n.id} />)}
      </div>
    </>
  )
}
